job-name-cybersmith = Cyber Smith
job-name-mechasapient = Mech Sapient
job-name-menial = Menial

job-name-bum = Bum
job-name-cleanser = Cleanser
job-name-hunter = Hunter
job-name-pilgrim = Pilgrim

job-name-apothecary = Apothecary
job-name-preacher = Preacher

job-name-enforcer-captain = Captain
job-name-enforcer-patrolman = Patrolman
job-name-enforcer-sergeant = Sergeant

job-name-guard = Guard
job-name-noble = Noble
job-name-worker = Worker

job-name-skitarii-alpha = Alpha
job-name-skitarii-provost = Provost
job-name-skitarii-unit = Unit

job-name-boss = Boss
job-name-barkeep = Barkeep
job-name-cook = Cook
job-name-bruiser = Bruiser
